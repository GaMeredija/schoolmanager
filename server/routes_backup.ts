...
const hasRole = (roles: string[]) => {
  ...
  // Criar evento
  app.post('/api/events', isAuthenticated, hasRole(['admin', 'teacher']), async (req, res) => {
    try {
      const {
        title,
        description,
  ...
  app.post('/api/admin/users', isAuthenticated, hasRole(['admin']), async (req, res) => {
    try {
      ...
      if (!role || !['student', 'teacher', 'admin'].includes(role)) {
        return res.status(400).json({ message: "Funcao invalida. Use: student, teacher ou admin" });
      }
  ...
  app.post('/api/admin/subjects', isAuthenticated, hasRole(['admin']), async (req, res) => {
    try {
      ...
      if (selectedClasses && selectedClasses.length > 0) {
        console.log("Vinculando disciplina às turmas: " + selectedClasses.join(', '));
    ...
    try {
      ...
      try {
        ...
        if (subjectsToLink.length > 0 && classesToLink.length > 0) {
          ...
          console.log("Vinculando professor " + newTeacher.id + " à disciplina " + firstSubject.name + " e turma " + firstClass.name);
          ...
          if (existingLink.length > 0) {
            ...
            console.log("Vinculo existente atualizado com professor! ID: " + existingLink[0].id);
          } else {
            // Criar novo v?nculo
            const classSubjectId = uuidv4();
            await db.insert(classSubjects).values({
            ...
            console.log("Novo vinculo criado com sucesso! ID: " + classSubjectId);
          }
        } else {
          console.log("Nao ha disciplinas ou turmas disponiveis para vincular");
        }
      } catch (linkError) {
  ...
  app.get('/api/materials/:materialId/download', isAuthenticated, async (req, res) => {
    try {
      ...
      // Se há apenas um arquivo, fazer download direto
      ...
      // Se há múltiplos arquivos, criar um ZIP
      const archiver = require('archiver');
      const archive = archiver('zip', { zlib: { level: 9 } });
  ...
  app.get('/api/student/exams', isAuthenticated, hasRole(['student']), async (req, res) => {
    try {
      ...
      if (studentClassData.length === 0) {
        console.log("Aluno não tem turma ativa");
        return res.json([]);
      }
      ...
      // Buscar notas do aluno para cada prova
      const examsWithGrades = await Promise.all(
        examsData.map(async (exam) => {
          const grade = await db
            ...
            .from(examGrades)
            .where(and(
              eq(examGrades.examId, exam.id),
              eq(examGrades.studentId, user.id)
            ))
            .limit(1);
  ...
  // Buscar eventos do calendário (provas e atividades)
  app.get('/api/calendar/events', isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const { startDate, endDate } = req.query;

      console.log("Buscando eventos do calendário para:", user.id);
      ...
      if (user.role === 'student') {
        ...
        if (studentClassData.length > 0) {
          ...
          events.push(...examsData.map(exam => ({
            ...exam,
            date: exam.examDate,
            color: '#ef4444', // Vermelho para provas
            icon: '📝'
          })));
        }
        ...
        const activitiesData = await db
          ...
          .from(activities)
          .innerJoin(subjects, eq(activities.subjectId, subjects.id))
          .innerJoin(studentClass, eq(studentClass.classId, activities.classId))
          .where(and(
            eq(studentClass.studentId, user.id),
            eq(studentClass.status, 'active'),
        ...
        events.push(...activitiesData.map(activity => ({
          ...
          color: '#3b82f6', // Azul para atividades
          icon: '📋'
        })));
      ...
      console.log("Encontrados " + events.length + " eventos do calendário");
      res.json(events);
    } catch (error) {
  ...
  app.get('/api/admin/classes/:id/details', isAuthenticated, hasRole(['admin']), async (req, res) => {
    try {
      ...
      // Buscar eventos criados pelo coordenador
      if (global.coordinatorEvents) {
        const coordinatorEvents = global.coordinatorEvents.filter((event: any) => {
        ...
        events.push(...coordinatorEvents);
      }

      // Eventos padrão (se não houver eventos criados)
      if (!global.coordinatorEvents || global.coordinatorEvents.length === 0) {
        const generalEvents = [
          {
            id: 'event-1',
            title: 'Reunião Pedagógica',
            description: 'Reunião mensal com professores',
      ...
      console.log(`Encontrados ${events.length} eventos para o coordenador`);
      res.json(events);
    } catch (error) {
      console.error('Erro ao buscar eventos do calendário:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
  ...
  app.post('/api/coordinator/calendar/events', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { title, description, date, type, subjectId, classId } = req.body;
      console.log("Criando evento do calendário:", { title, type, date });

      // Criar evento na tabela de eventos (se existir) ou simular
      const eventId = `event-${Date.now()}`;
      
      // Por enquanto, vamos simular a criação e armazenar em localStorage do servidor
      // Em uma implementação real, você criaria uma tabela de eventos
      const newEvent = {
        id: eventId,
        title,
        description,
        ...
        subjectId: subjectId || null,
        classId: classId || null,
        subjectName: subjectId ? 'Disciplina' : '',
        className: classId ? 'Turma' : '',
        teacherName: '',
        color: type === 'exam' ? '#ef4444' : type === 'activity' ? '#3b82f6' : '#8b5cf6',
        icon: type === 'exam' ? '📝' : type === 'activity' ? '📋' : '📅',
        createdAt: new Date().toISOString()
      };

      // Armazenar em uma variável global temporária (em produção, usar banco de dados)
      if (!global.coordinatorEvents) {
        global.coordinatorEvents = [];
      }
      global.coordinatorEvents.push(newEvent);
  ...
  app.put('/api/coordinator/calendar/events/:id', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { id } = req.params;
      const { title, description, date, type, subjectId, classId } = req.body;
      console.log("Atualizando evento do calendário:", id);

      // Atualizar evento na variável global
      if (global.coordinatorEvents) {
        const eventIndex = global.coordinatorEvents.findIndex((event: any) => event.id === id);
        if (eventIndex !== -1) {
          global.coordinatorEvents[eventIndex] = {
            ...global.coordinatorEvents[eventIndex],
            title,
            description,
            date,
  ...
  app.delete('/api/coordinator/calendar/events/:id', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { id } = req.params;
      console.log("Deletando evento do calendário:", id);

      // Remover evento da variável global
      if (global.coordinatorEvents) {
        global.coordinatorEvents = global.coordinatorEvents.filter((event: any) => event.id !== id);
      }
  ...
  app.get('/api/coordinator/system/status', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      console.log("Status do sistema solicitado pelo coordenador");

      // Contar registros
      const studentsCount = await db.select().from(users).where(eq(users.role, 'student'));
      const teachersCount = await db.select().from(users).where(eq(users.role, 'teacher'));
      const classesCount = await db.select().from(classes);
      const subjectsCount = await db.select().from(subjects);
      ...
      const systemStatus = {
        ...
        exams: examsCount.length,
        materials: materialsCount.length,
        storageUsed: 45.2, // Simulado
        activeUsers: 23, // Simulado
        systemLoad: 65, // Simulado
        memoryUsage: 78, // Simulado
        diskUsage: 45 // Simulado
      };
  ...
  app.get('/api/coordinator/system/limits', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      console.log("Limites do sistema solicitados pelo coordenador");

      // Por enquanto, retornar limites padrão
      // Em produção, estes valores viriam de uma tabela de configurações
      const systemLimits = {
        maxStudents: 1000,
        maxTeachers: 100,
        maxClasses: 50,
      ...
      res.json(systemLimits);
    } catch (error) {
      console.error('Erro ao obter limites do sistema:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });

  // Atualizar limites do sistema
  app.put('/api/coordinator/system/limits', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const limits = req.body;
      console.log("Atualizando limites do sistema:", limits);

      // Por enquanto, apenas simular a atualização
      // Em produção, salvaria em uma tabela de configurações
      console.log("Limites atualizados com sucesso");
      res.json({ message: "Limites atualizados com sucesso", limits });
    } catch (error) {
      console.error('Erro ao atualizar limites do sistema:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });

  // Executar limpeza do sistema
  app.post('/api/coordinator/system/cleanup', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { type } = req.body;
      console.log("Executando limpeza do sistema:", type);
      ...
      switch (type) {
        case 'old-data':
          ...
          break;
        case 'inactive-users':
          // Arquivar usuários inativos (simulado)
          cleanedCount = 8;
          break;
        case 'database':
          // Otimizar banco de dados (simulado)
          cleanedCount = 1;
          break;
        case 'temp-files':
          // Limpar arquivos temporários (simulado)
          cleanedCount = 25;
          break;
        default:
          return res.status(400).json({ message: "Tipo de limpeza inválido" });
      }

      console.log(`Limpeza executada: ${cleanedCount} itens processados`);
      res.json({ 
      ...
      console.error('Erro ao executar limpeza do sistema:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });

  // Bloquear/desbloquear criação de usuários
  app.post('/api/coordinator/system/toggle-user-creation', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { blocked } = req.body;
      console.log("Alterando bloqueio de criação de usuários:", blocked);

      // Por enquanto, apenas simular
      // Em produção, salvaria em uma tabela de configurações
      console.log(`Criação de usuários ${blocked ? 'bloqueada' : 'liberada'}`);
      res.json({ 
        message: `Criação de usuários ${blocked ? 'bloqueada' : 'liberada'}`, 
        blocked 
      });
    } catch (error) {
      console.error('Erro ao alterar bloqueio de criação de usuários:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });

  // Ativar/desativar modo de manutenção
  app.post('/api/coordinator/system/maintenance-mode', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { enabled } = req.body;
      console.log("Alterando modo de manutenção:", enabled);
  ...
  // Listar professores para supervisão
  app.get('/api/coordinator/teachers', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const teachers = await db
        .select({
          id: users.id,
      ...
      // Para cada professor, buscar suas turmas e disciplinas
      const teachersWithDetails = await Promise.all(
        teachers.map(async (teacher) => {
          const teacherClasses = await db
            ...
            .innerJoin(subjects, eq(classSubjects.subjectId, subjects.id))
            .where(and(
              eq(classSubjects.teacherId, teacher.id),
              eq(classSubjects.status, 'active')
            ));
          ...
          return {
            ...
            pendingGrades: 0, // Mockado por enquanto
            pendingAttendance: 0, // Mockado por enquanto
            pendingContent: 0, // Mockado por enquanto
            status: 'good' // Mockado por enquanto
          };
        })
      );
  ...
  // Listar alunos para supervisão
  app.get('/api/coordinator/students', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const students = await db
        .select({
          id: users.id,
      ...
      // Mockar dados de desempenho por enquanto
      const studentsWithPerformance = students.map(student => ({
        ...student,
        average: Math.random() * 4 + 6, // Entre 6 e 10
        attendance: Math.random() * 20 + 80, // Entre 80 e 100
        status: Math.random() > 0.7 ? 'warning' : 'good',
        risk: Math.random() > 0.8,
        subjects: [
          { name: 'Matemática', grade: Math.random() * 4 + 6, status: 'good' },
          { name: 'Português', grade: Math.random() * 4 + 6, status: 'good' },
          { name: 'História', grade: Math.random() * 4 + 6, status: 'good' }
        ]
      }));

      res.json({ data: studentsWithPerformance });
    } catch (error) {
  ...
  // Listar turmas para gerenciamento
  app.get('/api/coordinator/classes', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      ...
      // Para cada turma, buscar alunos e professores
      const classesWithDetails = await Promise.all(
        classesData.map(async (classItem) => {
          ...
          const students = await db
            ...
            .innerJoin(users, eq(studentClass.studentId, users.id))
            .where(and(
              eq(studentClass.classId, classItem.id),
              eq(studentClass.status, 'active')
            ));
          ...
          const teachers = await db
            ...
            .innerJoin(subjects, eq(classSubjects.subjectId, subjects.id))
            .where(and(
              eq(classSubjects.classId, classItem.id),
              eq(classSubjects.status, 'active')
            ));
          ...
          return {
            ...
            teachers: teachers,
            room: `Sala ${classItem.grade}${classItem.section}`,
            schedule: [
              { day: 'Segunda', time: '08:00-12:00', subject: 'Matemática' },
              { day: 'Terça', time: '08:00-12:00', subject: 'Português' },
              { day: 'Quarta', time: '08:00-12:00', subject: 'História' },
              { day: 'Quinta', time: '08:00-12:00', subject: 'Ciências' },
              { day: 'Sexta', time: '08:00-12:00', subject: 'Geografia' }
            ]
          };
        })
      );
  ...
  app.get('/api/coordinator/classes/:classId/schedule', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { classId } = req.params;
      console.log('Buscando horários da turma:', classId);
      ...
      const schedule = await db
        ...
        .leftJoin(users, eq(classSchedule.teacherId, users.id))
        .where(eq(classSchedule.classId, classId))
        .orderBy(classSchedule.day, classSchedule.startTime);

      console.log('Horários encontrados:', schedule.length);
  ...
  // Salvar horários de uma turma
  app.post('/api/coordinator/classes/:classId/schedule', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      const { classId } = req.params;
      const { schedule } = req.body;
      console.log('Salvando horários da turma:', classId, 'Total:', schedule.length);

      // Deletar horários existentes
      await db.delete(classSchedule).where(eq(classSchedule.classId, classId));
      ...
      if (schedule && schedule.length > 0) {
        ...
        await db.insert(classSchedule).values(scheduleData);
        console.log('Horários salvos com sucesso:', scheduleData.length);
      }

      res.json({ message: "Horários salvos com sucesso" });
    } catch (error) {
      console.error('Erro ao salvar horários da turma:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });

  // Obter alertas do sistema
  app.get('/api/coordinator/system/alerts', isAuthenticated, hasRole(['coordinator']), async (req, res) => {
    try {
      console.log("Alertas do sistema solicitados pelo coordenador");
      ...
      if (studentsCount.length > 900) {
        alerts.push({
          type: 'critical',
          message: `Limite de alunos quase atingido: ${studentsCount.length}/1000`,
          category: 'users'
        });
      }

      if (teachersCount.length > 80) {
        alerts.push({
          type: 'warning',
          message: `Muitos professores cadastrados: ${teachersCount.length}/100`,
          category: 'users'
        });
      }

      if (classesCount.length > 45) {
        alerts.push({
          type: 'warning',
          message: `Muitas turmas cadastradas: ${classesCount.length}/50`,
          category: 'classes'
        });
      }

      if (subjectsCount.length > 25) {
        alerts.push({
          type: 'warning',
          message: `Muitas disciplinas cadastradas: ${subjectsCount.length}/30`,
          category: 'subjects'
        });
      }

      // Verificar usuários inativos
      const inactiveUsers = await db.select().from(users).where(eq(users.status, 'inactive'));
      if (inactiveUsers.length > 50) {
        alerts.push({
          type: 'info',
          message: `${inactiveUsers.length} usuários inativos encontrados`,
          category: 'maintenance'
        });
      }

      console.log(`Encontrados ${alerts.length} alertas do sistema`);
      res.json(alerts);
    } catch (error) {
